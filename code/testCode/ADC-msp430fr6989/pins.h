...
#define SPI_CONFIG_SI_PIN_AS_OUTPUT()    st( P2DIR |=  BV(SPI_SI_GPIO_BIT__); )
#define SPI_DRIVE_SI_HIGH()              st( P2OUT |=  BV(SPI_SI_GPIO_BIT__); )
#define SPI_DRIVE_SI_LOW()               st( P2OUT &= ~BV(SPI_SI_GPIO_BIT__); )

/* SO Pin Configuration */
#define SPI_SO_GPIO_BIT__              1
#define SPI_CONFIG_SO_PIN_AS_INPUT()     st( P2DIR &= ~BV(SPI_SO_GPIO_BIT__);)
#define SPI_SO_IS_HIGH()                 ( P2IN & BV(SPI_SO_GPIO_BIT__) )

/* SPI Port Configuration */
#define SPI_CONFIG_PORT()                st( P2SEL1 &= ~(BV(SPI_SI_GPIO_BIT__)   |  \
                                                    BV(SPI_SO_GPIO_BIT__)| BV(SPI_SCLK_GPIO_BIT__)); \
                                              P2SEL0 |= BV(SPI_SI_GPIO_BIT__) | BV(SPI_SO_GPIO_BIT__) | BV(SPI_SCLK_GPIO_BIT__); \
                                               )
...
/* read/write macros */
#define SPI_WRITE_BYTE(x)                st( UCA0IFG &= ~UCRXIFG;  UCA0TXBUF = x; )
#define SPI_READ_BYTE()                  UCA0RXBUF
#define SPI_WAIT_DONE()                  while(!(UCA0IFG & UCRXIFG));
